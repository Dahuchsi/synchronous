...
}, [activeNoteId]); // This now ONLY runs when you click a new note
...
const handleGenerateRecipe = async () => {
  ...
  try {
    ...
    // --- FIX IS HERE ---
    // Manually update the editor's display since the useEffect won't run.
    if (editorRef.current) {
      editorRef.current.innerHTML = newContent;
    }

    // Update the state for the sidebar and prepare to save to DB
    const updatedNotes = notes.map(note =>
...
<style>{`
 ...
 /* CRITICAL FIX: Added .note-content to make selectors specific */
  .note-content .checklist-item {
    display: flex;
    align-items: center;
    margin-bottom: 0.5rem;
  }
  .note-content .checklist-checkbox {
    margin-right: 0.75rem; /* Space between box and text */
    width: 1rem;
    height: 1rem;
