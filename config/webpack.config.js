...
// === NEW: load .env so values exist at build time ===
...
// Detect Tailwind (prefer v4 plugin "@tailwindcss/postcss", fall back to "tailwindcss")
...
const useTailwind = Boolean(tailwindPlugin);
...
module.exports = function (webpackEnv) {
  ...
  // CRA’s default env loader
  const env = getClientEnvironment(paths.publicUrlOrPath.slice(0, -1));

  // === NEW: collect REACT_APP_* keys explicitly from dotenv ===
  ...
  return {
    ...
    plugins: [
      ...
      // === FIX: Inject both CRA + dotenv REACT_APP_* vars
